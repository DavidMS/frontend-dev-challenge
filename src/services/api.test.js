...
describe('api', () => {
    ...
    describe('getProducts', () => {
        it('fetches from the correct endpoint', async () => {
            fetch.mockResolvedValue({ ok: true, status: 200, json: () => Promise.resolve(mockProducts) })
            const result = await getProducts()
            expect(fetch).toHaveBeenCalledWith('https://itx-frontend-test.onrender.com/api/product', { signal: undefined })
        ...
        it('throws an ApiError and does not cache when the server responds with an error status', async () => {
            fetch.mockResolvedValue({ ok: false, status: 500, json: () => Promise.resolve({ message: 'boom' }) })
    ...
    describe('getProduct', () => {
        it('fetches from the correct endpoint with the id', async () => {
            fetch.mockResolvedValue({ ok: true, status: 200, json: () => Promise.resolve(mockProduct) })
            await getProduct('1')
            expect(fetch).toHaveBeenCalledWith('https://itx-frontend-test.onrender.com/api/product/1', { signal: undefined })
        })

        it('throws an ApiError and does not cache when the product is not found', async () => {
            fetch.mockResolvedValue({ ok: false, status: 404, json: () => Promise.resolve({ message: 'not found' }) })
    ...
    describe('addToCart', () => {
        it('posts to the cart endpoint with the correct body', async () => {
            fetch.mockResolvedValue({ ok: true, status: 200, json: () => Promise.resolve({ count: 1 }) })
        ...
        it('throws an ApiError when the server rejects the request', async () => {
            fetch.mockResolvedValue({ ok: false, status: 400, json: () => Promise.resolve({ message: 'invalid' }) })
            await expect(addToCart({ id: '1', colorCode: 1000, storageCode: 2000 })).rejects.toThrow(ApiError)
        })
    })
})